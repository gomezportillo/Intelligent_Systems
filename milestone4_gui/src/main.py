...
class MyApp:
    ...
    def __init__(self, init_node, obj_nodes, coordinates):

        initial_state = State(Node_Map(init_node), obj_nodes)
        print "Looking for nodes " + str(obj_nodes)    
        boundary_coordinates = coordinates
        self.problem = Problem(State_Space(boundary_coordinates), initial_state)
    
    def build_hash_table(self):
        return self.problem.build_hash_table()

    def search_iface(self, strategy, max_depth=10000, incr_depth=1):

        self.path = self.search(self.problem, strategy, max_depth, incr_depth)


    def search(self, problem, strategy, max_depth=10000, incr_depth=1):
    ...
    def bounded_search(self, problem, strategy, max_depth=10000):
    ...
    def create_solution(self, final_node):
